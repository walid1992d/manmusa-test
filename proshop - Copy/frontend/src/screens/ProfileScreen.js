...
const ProfileScreen = ({ location, history }) => {
  ...
  const submitHandler = e => {
    ...
    if (password !== confirmPassword) {
      ...
      dispatch(updateUserProfile({ id: user._id, name, email, password, number, address }));
    }
  };

  return (
    <Row>
      {userInfo._id !== '613891b3cb025667fc50de38' ? (
        <>
          <Col md={3}>
        ...
        <>
          <h5>Please login or register to view orders </h5>
        </>
      )}
    </Row>
  );
